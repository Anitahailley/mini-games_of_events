...
void g_m(double materials[])
{
	...
	materials[7] = materials[4] / 5 + materials[4] * 1.6 + materials[7];		//额外产出 1.6倍沙伊纳
...
void g_l(double materials[])
{
	...
	materials[7] = materials[5] * 3 / 10 + materials[5] * 1.4 + materials[7];	//额外产出 1.4倍沙伊纳
	materials[7] = ceil(materials[7]);
...
int main(void)
{
	...
	while (1)
	{
		...
		printf("\n请输入加工工艺：\n");						//工艺全解锁后，从左到右数，第四个为第一个加工工艺，以此类推
		for (int i = 0; i < 6; i++)						//工艺均为MAX
		{
			printf("第%d个操作台：", i + 1);
			scanf_s("%d", a + i);
